...
def minimise_energy_bosonic(params):
    """
    ...
    while error > params.tol_ks:
        ...
        density_in = newton_mixing(params, density_in, density_out - density_in,
...
def converge_bosonic_system(params, density):

    density2 = np.copy(density)

    a = params.num_particles
    params.num_particles = 1

    # Optionally use scipy's rootfinder
    opt_info = sp.optimize.root(ks_objective_function_bosonic, density, args=(params), method='linearmixing', tol=1e-12)

    params.num_particles = a
    #delta_rho = opt_info.x - density2

    return opt_info.x


def ks_objective_function_bosonic(density_in, params, precond=None):
    """
    ...
    # Extract lowest lying num_particles eigenfunctions and normalise
    wavefunctions_ks = eigenvectors[:,0]
    wavefunctions_ks[:] = normalise_function(params, wavefunctions_ks)

    # Calculate the output density
    density_out = abs(wavefunctions_ks[:])**2
